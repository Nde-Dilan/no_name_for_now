...
class TranslationManager {
  ...
  async callTranslationApi(sourceLang, targetLang, text) {
    const cleanSourceLang = sourceLang.toLowerCase().replace("'", "").replace("á", "a");
    const cleanTargetLang = targetLang.toLowerCase().replace("'", "").replace("á", "a");
    try {
  ...
  async handleTranslation() {
    ...
    const sourceLang = sourceLanguageBtn ? 
      sourceLanguageBtn.textContent.trim().split(" ")[0].toLowerCase() : "french";
    const targetLang = targetLanguageBtn ? 
      targetLanguageBtn.textContent.trim().split(" ")[0].toLowerCase() : "ghomala";
    
    // Get text to translate
    const textToTranslate = this.translationInput ? this.translationInput.value.trim() : "";
    ...
    try {
      ...
      const translationResult = await this.callTranslationApi(sourceLang, targetLang, textToTranslate);
...
function populateResultsPage() {
  ...
  if (sourceLanguageElement) {
    sourceLanguageElement.textContent = currentTranslation.sourceLang.charAt(0).toUpperCase() + 
                                        currentTranslation.sourceLang.slice(1);
  }
  
  if (targetLanguageElement) {
    targetLanguageElement.textContent = currentTranslation.targetLang.charAt(0).toUpperCase() + 
